...
const PortfolioCard = ({ project, variants }) => {
    return (
        <motion.div
            className="bg-white bg-opacity-5 rounded-lg p-6 sm:p-10 flex flex-col md:flex-row items-center gap-8 md:gap-12 w-full shadow-xl border border-white border-opacity-5"
            variants={variants}
        >
            ...
            <div className="flex flex-col gap-4 items-center md:items-start w-full md:w-1/2">
                ...
                <div className="flex gap-2 flex-wrap justify-center md:justify-start w-full">
                    {project.fields.tags.map((tag) => (
                        <span
                            key={tag.fields.name}
                            className="flex items-center gap-1 bg-white bg-opacity-10 py-1 px-2.5 rounded-full text-xs border border-white border-opacity-10"
                        >
                            {tag.fields.icon && (
                                <img
